...
checkpoint_dir = Path("checkpoints/Secbone/llama-2-13B-instructed")
...
async def process_request(input_data: ProcessRequest) -> ProcessResponse:
    ...
    logprobs = torch.max(torch.nn.functional.log_softmax(torch.stack(scores),dim=-1),dim=-1)[0]
    ...
    generated_tokens = tokens.cpu().numpy()[0][prompt_length:].tolist()
    logprobs_sum = sum(logprobs).cpu().numpy()[0]
